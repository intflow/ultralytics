...
# 일반 yolo 8m 모델  로드    
model = YOLO("/works/ultralytics/yolov8n.pt")
...
# torch.save(model.state_dict(), 'yolov8_qdc.pth')
# 다시 load
model=YOLO('yolov8n_qat.pt')
model.export(format='onnx')  # creates 'yolov8s.onnx'
# 다시 train 
# model.train(data="coco8.yaml",epochs=200)
...
# torch.onnx.export( model.model, dummy_input, "quant_yolo.onnx",  opset_version=13)
